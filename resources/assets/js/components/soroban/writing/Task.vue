<template>
    <div>
        <div class="phrase-reading__description" v-if="!isStarted">
            ...
            <button class="btn">
                <router-link to="/paraphrase/writing">Настройки</router-link>
            </button>
            <button class="btn btn-primary" @click="next">Далее</button>
        </div>
        <div class="phrase-reading__task" v-if="isStarted">
            <h2>{{question}}</h2>
            <soroban :value="max" :isClickedTask="true" @totalCalculated="setTotal"></soroban>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            Store.setWritingQuestion(this.question);
        ...
        methods: {
            ...
            setTotal(total) {
                this.answer = total;
                Store.setWritingAnswer(total);
            },
            sendAnswer() {
                Store.setWritingAnswer(this.answer);
                this.$router.push('/soroban/writing/result');
            }
        }
    }
</script>